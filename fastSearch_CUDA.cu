...
{
    if (argc != 5){
        printf("%s\n", "Must input 4 arguements, in order, input database, input queries, output file, int block size.");
        printf("%s\n", "Example run: fastSearch_CUDA mols.bin inq.bin search.txt 1024");
        exit(EXIT_FAILURE); 
    }
